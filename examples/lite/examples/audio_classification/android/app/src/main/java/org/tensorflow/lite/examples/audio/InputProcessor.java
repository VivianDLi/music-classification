...
public class InputProcessor implements AudioProcessor {
  ...
  public InputProcessor(float sampleRate, int bufferSize, float inferenceSeconds,
      ByteBuffer outputBuffer, ConstantQ cqt) {
    this.sampleRate = sampleRate;
    this.bufferSize = bufferSize;
    this.cqt = cqt;
    this.outputBuffer = outputBuffer;
    int outputBufferSize =
        (int) Math.ceil(84 * 2 * (sampleRate * inferenceSeconds / bufferSize)) * 4;
  ...
  public boolean process(AudioEvent audioEvent) {
    ...
    if (this.backgroundBuffer.limit() == this.backgroundBuffer.capacity()) {
      try {
        ...
        Timber.tag("InputProcessor")
            .e("Buffer overflow exception with error %s, resetting all buffers.", e.getMessage());
        e.printStackTrace();
        this.foregroundBuffer.clear();
        this.backgroundBuffer.clear();
      } catch (UnsupportedOperationException e) {
        Timber.tag("InputProcessor").e("Failed to transpose foreground buffer with error %s.",
            e.getMessage());
        e.printStackTrace();
      }
    }
